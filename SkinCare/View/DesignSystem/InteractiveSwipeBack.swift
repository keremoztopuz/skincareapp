...
private struct EdgeSwipeToDismiss: ViewModifier {
    ...
    /// Only the leading strip listens, so vertical scrolling and the
    /// horizontal product rails keep every touch that starts inside them.
    private static let edgeWidth: CGFloat = 20
    private static let dismissThreshold: CGFloat = 90
    private static let flickThreshold: CGFloat = 200
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 10)
                            ...
                            .onEnded { value in
                                ...
                                if offset > Self.dismissThreshold || projected > Self.flickThreshold {
                                    // No exit animation of our own: the cover
                                    // plays its own, and running both makes
                                    // the content jump.
                                    onDismiss()
                                } else {
